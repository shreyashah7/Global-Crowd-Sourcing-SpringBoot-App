...
class MainContent extends Component {
    ...
    componentDidMount() {
        ...
        if (!!userObj) {
            let userInfo = { id: userObj.id, role: userObj.role };
            ...
            return (
                <tr key={index + 1}>
                    ...
                    {this.state.user.role === 1 &&
                        <td className="text-left">{'$' + project.job_rate + '/' + UserHelper.getJobType(project.job_type)}</td>
                    }
                    {this.state.user.role === 2 &&
                        <td className="text-left">{'$' + project.bid_rate + '/' + UserHelper.getJobType(project.bid_type)}</td>
                    }
                    <td >
                        <Timestamp time={project.created_at} format='date' /><br />
                        <small className="text-muted"><Timestamp time={project.created_at} precision={1} /></small>
                    </td>
                </tr>
            )
        });
